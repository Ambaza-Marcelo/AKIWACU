...
@section('title')
@lang('Drink Big Store Detail') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Drink Big Store Detail : '){{ $drink_big_store->code }}</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">@lang('Drink Big Store Detail : '){{ $drink_big_store->code }}</h4>
                    ...
                    <div class="data-tables">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>@if($drink_big_store->drink_id){{ $drink_big_store->drink->name }} @endif</td>
                                    <td>@if($drink_big_store->drink_id){{ $drink_big_store->drink->code }} @endif</td>
                                    <td>{{ $drink_big_store->code }}</td>
                                    @if($drink_big_store->quantity_bottle <= $drink_big_store->threshold_quantity)
                                    <td>{{ $drink_big_store->quantity_bottle }}<img src="{{ asset('img/warning.gif')}}" width="30"></td>
                                    @else
                                    <td>{{ $drink_big_store->quantity_bottle }}</td>
                                    @endif
                                    <td>{{ $drink_big_store->drink->drinkMeasurement->purchase_unit }}</td>
                                    <td>{{ $drink_big_store->threshold_quantity }}</td>
                                    <td>{{ number_format($drink_big_store->cump,0,',',' ') }}</td>
                                    <td>{{ number_format(($drink_big_store->cump * $drink_big_store->quantity_bottle),0,',',' ') }}</td>
                                    <!--
