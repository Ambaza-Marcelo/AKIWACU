...
@section('title')
@lang('RAPPORT DU STOCK DES NOURRITURES (GRAND STOCK) VERS LA BARRIST') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('RAPPORT DU STOCK DES NOURRITURES (GRAND STOCK) VERS LA BARRIST')</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('admin.barrist-drink-big-report.export-to-excel')}}" method="GET">
                    ...
                    <form action="{{ route('admin.barrist-drink-big-report.export-to-pdf')}}" method="GET">
                        <p class="float-right mb-2">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td>@if($data->quantity_stockin){{ $data->quantity_stockin }} {{$data->unit}} @elseif($data->quantity_reception) {{ $data->quantity_reception }} {{$data->unit}} @endif </td>
                                    <td>@if($data->value_stockin){{ $data->value_stockin }} @elseif($data->value_reception) {{ $data->value_reception }} @endif </td>
                                    <td>@if($data->quantity_stockout){{ $data->quantity_stockout }} {{$data->unit}} @elseif($data->quantity_transfer) {{ $data->quantity_transfer }} {{$data->unit}} @endif </td>
                                    <td>@if($data->value_stockout){{ $data->value_stockout }} @elseif($data->value_transfer) {{ $data->value_transfer }} @endif </td>
                                    <td>{{ $data->quantity_stock_final }} {{ $data->unit }}</td>
