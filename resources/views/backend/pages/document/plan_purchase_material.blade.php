            ...
            <div>
                ...
                <div>
                    ...
                    <div style="float: right; border-top-right-radius: 10px solid black;border-top-left-radius: 10px solid black;border-bottom-right-radius: 10px solid black;border-bottom-left-radius: 10px solid black; background-color: rgb(150,150,150);width: 242px;padding: 20px;">
                        <small>
                           &nbsp;&nbsp; <img src="data:image/png;base64, {!! base64_encode(QrCode::size(50)->generate('eSIGNATURE : '.$plan_signature.' www.edengardenresorts.bi')) !!} ">
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Plan Number: {{ $plan_no }}
                        </small><br>
                    ...
                    <br><br><br>
                    <div>
                        <h3 style="text-align: center;text-decoration: underline;">PLAN D'APPROVISIONNEMENT DU {{ \Carbon\Carbon::parse($start_date)->format('d/m/Y') }} AU {{ \Carbon\Carbon::parse($end_date)->format('d/m/Y') }}</h3>
                    </div>
                    <div>
                        <table style="border: 1px solid black;border-collapse: collapse;">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td>{{ $data->quantity }}</td>
                                    <td>{{ $data->unit }}</td>
                                    <td>{{ number_format($data->material->purchase_price,0,',',' ') }}</td>
                                    <td>{{ number_format($data->purchase_price,0,',',' ') }}</td>
                                    <td>{{ number_format($data->total_purchase_amount,0,',',' ') }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                            <tfoot>
