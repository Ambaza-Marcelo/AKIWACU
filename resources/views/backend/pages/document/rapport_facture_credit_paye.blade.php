            ...
            <div>
               <div>
                ...
                <div>
                    ...
                    <br>
                    <div>
                        <h2 style="text-align: center;text-decoration: underline;">JOURNAL DES CREDITS PAYES DU {{ \Carbon\Carbon::parse($start_date)->format('d/m/Y') }} AU {{ \Carbon\Carbon::parse($end_date)->format('d/m/Y') }} </h2>
                    </div>
                    <br>
                    <div>
                        <table style="border: 1px solid black;border-collapse: collapse;">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td>@if($data->drink_order_no){{ $data->drink_order_no }}@elseif($data->food_order_no){{ $data->food_order_no }}@elseif($data->barrist_order_no){{ $data->barrist_order_no }}@elseif($data->bartender_order_no){{ $data->bartender_order_no }} @else {{ $data->booking_no }} @endif</td>
                                    <td>@if($data->client_id){{ $data->client->customer_name }} @else {{ $data->customer_name }} @endif</td>
                                    <td>@if($data->food_item_id){{ $data->foodItem->name }} @elseif($data->drink_id) {{ $data->drink->name }} @elseif($data->barrist_item_id) {{ $data->barristItem->name }} @elseif($data->bartender_item_id) {{ $data->bartenderItem->name }} @endif</td>
