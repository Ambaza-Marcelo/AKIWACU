...
@section('title')
@lang('Barrist store') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Barrist store')</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">@lang('Barrist store')</h4>
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('barrist_item.create'))
                            <a class="btn btn-primary text-white" href="{{ route('admin.barrist-transformation.create') }}">@lang('Saisie Production')</a>&nbsp;&nbsp;
                        @endif
                    </p>
                    ...
                    <div class="data-tables">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>{{ $stock->barristItem->name }}</td>
                                    <td>{{ $stock->barristItem->code }}</td>
                                    ...
                                    <td>
                                    @if (Auth::guard('admin')->user()->can('barrist_production_store.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.barrist-production-store.destroy', $stock->id) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $stock->id }}').submit();">
                                                Delete
                                            </a>

                                            <form id="delete-form-{{ $stock->id }}" action="{{ route('admin.barrist-production-store.destroy', $stock->id) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
