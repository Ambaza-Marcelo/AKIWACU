...
@section('title')
@lang('Validation de facture à cash') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Validation de facture à cash')</h4>
                <ul class="breadcrumbs pull-left">
                    <li><a href="{{ route('admin.dashboard') }}">@lang('messages.dashboard')</a></li>
                    <li><a href="{{ route('ebms_api.invoices.index') }}">@lang('messages.list')</a></li>
                    <li><span>@lang('Validation de facture à cash')</span></li>
                </ul>
            </div>
        </div>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Validation de facture à cash : {{$facture->invoice_number}}</h4>
                    @include('backend.layouts.partials.messages')
                    
                    <form action="@if($facture->drink_order_no){{ route('admin.facture-boisson.validate', $facture->invoice_number) }} @elseif($facture->food_order_no){{ route('admin.facture-cuisine.validate', $facture->invoice_number) }} @elseif($facture->barrist_order_no){{ route('admin.facture-barrist.validate', $facture->invoice_number) }} @elseif($facture->bartender_order_no){{ route('admin.facture-bartender.validate', $facture->invoice_number) }} @elseif($facture->booking_no){{ route('admin.facture-booking.validate', $facture->invoice_number) }} @endif" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <button  type="submit" onclick="this.style.visibility='hidden';" ondblclick="this.style.visibility='hidden';" class="btn btn-primary mt-4 pr-4 pl-4">VALIDER A CASH</button>
                    </form>
                </div>
            </div>
        </div>
