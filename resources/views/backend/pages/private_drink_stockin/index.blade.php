            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.create'))
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>{{ $stockin->date }}</td>
                                    <td><a href="{{ route('admin.private-drink-stockins.show',$stockin->stockin_no)}}">{{ $stockin->stockin_no }}</a></td>
                                    ...
                                    <td>
                                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.validate'))
                                        @if($stockin->status == 1)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.private-drink-stockins.validate', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $stockin->stockin_no }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.validate', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.confirm'))
                                        @if($stockin->status == 2)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.private-drink-stockins.confirm', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('confirm-form-{{ $stockin->stockin_no }}').submit();">
                                                Confirmer
                                            </a>

                                            <form id="confirm-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.confirm', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if($stockin->status == 3)
                                            ...
                                            <form id="approuve-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.approuve', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.reject'))
                                            @if($stockin->status == 1 || $stockin->status == 2 || $stockin->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.private-drink-stockins.reject', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reject-form-{{ $stockin->stockin_no }}').submit();">
                                                Rejeter
                                            </a>
                                            @endif
                                            <form id="reject-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.reject', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.reset'))
                                            @if($stockin->status == -1 || $stockin->status == 2 || $stockin->status == 3 || $stockin->status == 4)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.private-drink-stockins.reset', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $stockin->stockin_no }}').submit();">
                                                Annuler
                                            </a>
                                            @endif
                                            <form id="reset-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.reset', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('private_drink_stockin.delete'))
                                            @if($stockin->status == -1 || $stockin->status == 1)
                                            <a class="btn btn-danger text-white" href="{{ route('admin.private-drink-stockins.destroy', $stockin->stockin_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $stockin->stockin_no }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $stockin->stockin_no }}" action="{{ route('admin.private-drink-stockins.destroy', $stockin->stockin_no) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                            @endif
                                        ...
                                        <a href="{{ route('admin.private-drink-stockins.bonEntree',$stockin->stockin_no) }}"><img src="{{ asset('img/ISSh.gif') }}" width="60" title="Télécharger d'abord le document et puis imprimer"></a>
