...
@section('title')
@lang('Rejeter la Commande Bartender') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Rejeter la Commande Bartender')</h4>
                <ul class="breadcrumbs pull-left">
                    <li><a href="{{ route('admin.dashboard') }}">@lang('messages.dashboard')</a></li>
                    <li><a href="{{ route('admin.f-bartender-orders.index',$data->table_id) }}">@lang('messages.list')</a></li>
                    <li><span>@lang('Rejeter la Commande Bartender')</span></li>
                </ul>
            </div>
        </div>
    ...
    <div class="row">
        <!-- data table start -->
         <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Rejeter la Commande Bartender</h4>
                    @include('backend.layouts.partials.messages')
                    
                    <form action="{{ route('admin.f-bartender-orders.reject', $data->order_no) }}" method="post" id="dynamic_form">
                        @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        ...
                        <div class="form-group">
                         ...
                         <table class="table table-bordered" id="dynamicTable">  
                            ...
                            <tr>  
                                <td><select class="form-control" name="bartender_item_id[]" id="bartender_item_id" required>
                                <option disabled="disabled" selected="selected">merci de choisir</option>
                                @foreach($articles as $article)
                                <option value="{{ $article->id }}" disabled class="form-control" {{ $data->bartender_item_id == $article->id ? 'selected' : '' }}>{{ $article->name }}/{{$article->code }}</option>
                                @endforeach
                                </select></td>  
