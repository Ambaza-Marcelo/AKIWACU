                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td><a href="{{ route('admin.food-big-store.show',$food_big_store->code) }}">{{ $food_big_store->code }}</a> </td>
                                    <td>{{ $food_big_store->store_signature }}</td>
                                    <td>{{ $food_big_store->emplacement }}</td>
                                    <td>{{ $food_big_store->manager }}</td>
                                    <td>
                                        <a href="{{ route('admin.food-big-store.storeStatus',$food_big_store->code) }}"><img src="{{ asset('img/ISSh.gif') }}" width="60" title="Télécharger d'abord le document et puis imprimer"></a>
                                        <a href="{{ route('admin.food-big-store.exportToExcel',$food_big_store->code) }}" class="btn btn-primary">Excel</a>
                                        @if (Auth::guard('admin')->user()->can('food_big_store.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.food-big-store.edit', $food_big_store->id) }}">@lang('messages.edit')</a>
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('food_big_inventory.create'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.food-big-store-inventory.create', $food_big_store->code) }}">@lang('messages.inventory')</a>
                                        @endif
