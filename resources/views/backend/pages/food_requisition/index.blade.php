                ...
                <div class="card-body">
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('food_requisition.create'))
                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.create') }}">@lang('messages.new')</a>
                        @endif
                    </p>
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td>
                                        ...
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('food_requisition.validate'))
                                        @if($requisition->status == 1 || $requisition->status == -1)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.validate', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $requisition->requisition_no }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.validate', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('food_requisition.confirm'))
                                        @if($requisition->status == 2)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.confirm', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('confirm-form-{{ $requisition->requisition_no }}').submit();">
                                                Confirmer
                                            </a>

                                            <form id="confirm-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.confirm', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('food_requisition.approuve'))
                                        @if($requisition->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.approuve', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('approuve-form-{{ $requisition->requisition_no }}').submit();">
                                                Approuver
                                            </a>

                                            <form id="approuve-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.approuve', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('food_requisition.reject'))
                                            @if($requisition->status == 1 || $requisition->status == 2 || $requisition->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.reject', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reject-form-{{ $requisition->requisition_no }}').submit();">
                                                Rejeter
                                            </a>
                                            @endif
                                            <form id="reject-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.reject', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('food_requisition.reset'))
                                            @if($requisition->status == -1 || $requisition->status == 2 || $requisition->status == 3 || $requisition->status == 4)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.food-requisitions.reset', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $requisition->requisition_no }}').submit();">
                                                Annuler
                                            </a>
                                            @endif
                                            <form id="reset-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.reset', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if (Auth::guard('admin')->user()->can('food_requisition.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.food-requisitions.edit', $requisition->requisition_no) }}">@lang('messages.edit')</a>
                                        @endif
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('food_requisition.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.food-requisitions.destroy', $requisition->requisition_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $requisition->requisition_no }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $requisition->requisition_no }}" action="{{ route('admin.food-requisitions.destroy', $requisition->requisition_no) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
