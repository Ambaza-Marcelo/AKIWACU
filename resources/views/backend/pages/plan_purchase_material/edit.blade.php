...
@section('title')
@lang('Modifier le planning des approvisionnements des materiels') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Modifier le planning des approvisionnements des materiels')</h4>
                <ul class="breadcrumbs pull-left">
                    <li><a href="{{ route('admin.dashboard') }}">@lang('messages.dashboard')</a></li>
                    <li><a href="{{ route('admin.plan-purchase-materials.index') }}">@lang('messages.list')</a></li>
                    <li><span>@lang('Modifier le planning des approvisionnements des materiels')</span></li>
                </ul>
            </div>
        </div>
    ...
    <div class="row">
        <!-- data table start -->
         <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title">Modifier le planning des approvisionnements des materiels</h4>
                    @include('backend.layouts.partials.messages')
                    
                    <form action="{{ route('admin.plan-purchase-materials.update',$plan->plan_no) }}" method="post" id="dynamic_form">
                        @csrf
                        @method('PUT')
                    <div class="row">
                        ...
                        <div class="col-md-6" id="dynamicDiv">
                         ...
                         <table class="table table-bordered" id="dynamicTable">  
                            ...
                            <tr class="">  
                                ...
                                <option value="{{ $material->id }}" {{ $plan->material_id == $material->id ? 'selected' : '' }} class="form-control">{{$material->name}}/{{ $material->code }}</option>
                                @endforeach
                                </select></td>  
                                ...
                                <td><select class="form-control" name="unit[]" id="unit">
                                        <option disabled="disabled" selected="selected">Merci de choisir</option>
                                        <option value="pcs" {{ $plan->unit == 'pcs' ? 'selected' : '' }} class="form-control">Pieces</option>
                                        <option value="boites" {{ $plan->unit == 'boites' ? 'selected' : '' }} class="form-control">Boites</option>
                                        <option value="kg" {{ $plan->unit == 'kg' ? 'selected' : '' }} class="form-control">KG</option>
                                        <option value="paire" {{ $plan->unit == 'paire' ? 'selected' : '' }} class="form-control">PAIRE</option>
