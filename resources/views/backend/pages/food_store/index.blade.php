...
@section('title')
@lang('Food store') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Food store')</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">@lang('Food store')</h4>
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('food_item.create'))
                            <a class="btn btn-primary text-white" href="{{ route('admin.food-transformation.create') }}">@lang('Saisie Production')</a>&nbsp;&nbsp;
                        @endif
                    </p>
                    ...
                    <div class="data-tables">
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    <td>{{ $loop->index+1 }}</td>
                                    <td>{{ $stock->foodItem->name }}</td>
                                    <td>{{ $stock->foodItem->code }}</td>
                                    ...
                                    <td>
                                    @if (Auth::guard('admin')->user()->can('food_store.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.food-store.destroy', $stock->id) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $stock->id }}').submit();">
                                                Delete
                                            </a>

                                            <form id="delete-form-{{ $stock->id }}" action="{{ route('admin.food-store.destroy', $stock->id) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
