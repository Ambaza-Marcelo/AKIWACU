                ...
                <div class="card-body">
                    ...
                    @if ($in_pending > 0)
                        <p class="float-right mb-2">
                            <a href="{{ route('admin.bartender-invoices.create-by-table',$table_id) }}" class="btn btn-success">Cloturer toute la table</a>
                        </p>
                        <p class="float-right mb-2">
                            <div class="row">
                                <div class="col-md-12">
                               ...
                               <tr>
                                    ...
                                    <td>
                                        ...
                                        @if($order->status == 0 || $order->status == -1)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.bartender-orders.validate', $order->order_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $order->order_no }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $order->order_no }}" action="{{ route('admin.bartender-orders.validate', $order->order_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if (Auth::guard('admin')->user()->can('drink_order_client.reset'))
                                            ...
                                            <a class="btn btn-primary text-white" href="{{ route('admin.bartender-orders.reset', $order->order_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $order->order_no }}').submit();">
                                                Annuler
                                            </a>
                                            @endif
                                            <form id="reset-form-{{ $order->order_no }}" action="{{ route('admin.bartender-orders.reset', $order->order_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        ...
                                        @if (Auth::guard('admin')->user()->can('drink_order_client.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.bartender-orders.edit', $order->id) }}">@lang('messages.edit')</a>
                                        @endif
                                        @endif
                                        @if($order->status == 1 && $order->flag == 1)
                                        @if (Auth::guard('admin')->user()->can('invoice_drink.create'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.bartender-invoices.create', $order->order_no) }}">@lang('Facturation')</a>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_order_client.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.bartender-orders.destroy', $order->order_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $order->order_no }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $order->order_no }}" action="{{ route('admin.bartender-orders.destroy', $order->order_no) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
