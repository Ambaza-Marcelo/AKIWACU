            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('drink_stockout.create'))
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    <td>
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.validate'))
                                        @if($stockout->status == 1)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.drink-stockouts.validate', $stockout->stockout_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('validate-form-{{ $stockout->stockout_no }}').submit();">
                                                Valider
                                            </a>

                                            <form id="validate-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.validate', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.confirm'))
                                        @if($stockout->status == 2)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.drink-stockouts.confirm', $stockout->stockout_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('confirm-form-{{ $stockout->stockout_no }}').submit();">
                                                Confirmer
                                            </a>

                                            <form id="confirm-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.confirm', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.approuve'))
                                        @if($stockout->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.drink-stockouts.approuve', $stockout->stockout_no) }}"
                                            ...
                                            <form id="approuve-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.approuve', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.reject'))
                                            @if($stockout->status == 1 || $stockout->status == 2 || $stockout->status == 3)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.drink-stockouts.reject', $stockout->stockout_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reject-form-{{ $stockout->stockout_no }}').submit();">
                                                Rejeter
                                            </a>
                                            @endif
                                            <form id="reject-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.reject', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.reset'))
                                            @if($stockout->status == -1 || $stockout->status == 2 || $stockout->status == 3 || $stockout->status == 4)
                                            <a class="btn btn-primary text-white" href="{{ route('admin.drink-stockouts.reset', $stockout->stockout_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('reset-form-{{ $stockout->stockout_no }}').submit();">
                                                Annuler
                                            </a>
                                            @endif
                                            <form id="reset-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.reset', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('PUT')
                                                @csrf
                                            </form>
                                        @endif
                                        @if (Auth::guard('admin')->user()->can('drink_stockout.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.drink-stockouts.edit', $stockout->stockout_no) }}">@lang('messages.edit')</a>
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('drink_stockout.delete'))
                                            @if($stockout->status == -1 || $stockout->status == 1)
                                            <a class="btn btn-danger text-white" href="{{ route('admin.drink-stockouts.destroy', $stockout->stockout_no) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $stockout->stockout_no }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $stockout->stockout_no }}" action="{{ route('admin.drink-stockouts.destroy', $stockout->stockout_no) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                            @endif
                                        ...
                                        <a href="{{ route('admin.drink-stockouts.bonSortie',$stockout->stockout_no) }}"><img src="{{ asset('img/ISSh.gif') }}" width="60" title="Télécharger d'abord le document et puis imprimer"></a>
