                ...
                <div class="card-body">
                    ...
                    <p class="float-right mb-2">
                        @if (Auth::guard('admin')->user()->can('booking_client.create'))
                            <a class="btn btn-primary text-white" href="{{ route('admin.booking-clients.create') }}">@lang('messages.new')</a>
                        @endif
                    </p>
                               ...
                               <tr>
                                    ...
                                    <td>
                                        @if (Auth::guard('admin')->user()->can('booking_client.edit'))
                                            <a class="btn btn-success text-white" href="{{ route('admin.booking-clients.edit', $client->id) }}">@lang('messages.edit')</a>
                                        @endif

                                        @if (Auth::guard('admin')->user()->can('booking_client.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.booking-clients.destroy', $client->id) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $client->id }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $client->id }}" action="{{ route('admin.booking-clients.destroy', $client->id) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
