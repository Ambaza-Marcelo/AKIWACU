...
@section('title')
@lang('Material Big Store Detail') - @lang('messages.admin_panel')
@endsection
    ...
    <div class="row align-items-center">
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                <h4 class="page-title pull-left">@lang('Material Big Store Detail : '){{ $material_big_store->code }}</h4>
...
<div class="main-content-inner">
    <div class="row">
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    <h4 class="header-title float-left">@lang('Material Big Store Detail : '){{ $material_big_store->code }}</h4>
                        ...
                        <table id="dataTable" class="text-center">
                            ...
                            <tbody>
                               ...
                               <tr>
                                    ...
                                    @else
                                    <td>{{ $material_big_store->quantity }}</td>
                                    @endif
                                    <td>{{ $material_big_store->unit }}</td>
                                    <td>{{ $material_big_store->threshold_quantity }}</td>
                                    <td>{{ number_format($material_big_store->purchase_price,0,',',' ') }}</td>
                                    <td>{{ number_format($material_big_store->total_purchase_value,0,',',' ') }}</td>
                                    <td>

                                        @if (Auth::guard('admin')->user()->can('material_big_store.delete'))
                                            <a class="btn btn-danger text-white" href="{{ route('admin.sotb-material-bg-store.destroy', $material_big_store->id) }}"
                                            onclick="event.preventDefault(); document.getElementById('delete-form-{{ $material_big_store->id }}').submit();">
                                                @lang('messages.delete')
                                            </a>

                                            <form id="delete-form-{{ $material_big_store->id }}" action="{{ route('admin.sotb-material-bg-store.destroy', $material_big_store->id) }}" method="POST" style="display: none;">
                                                @method('DELETE')
                                                @csrf
                                            </form>
                                        @endif
