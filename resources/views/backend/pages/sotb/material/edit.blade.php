        ...
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                ...
                <ul class="breadcrumbs pull-left">
                    ...
                    <li><a href="{{ route('admin.sotb-materials.index') }}">@lang('messages.list')</a></li>
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.sotb-materials.update',$material->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="store_type" id="store_type">
                                        <option disabled="disabled" selected="selected">Merci de choisir</option>
                                        <option value="0" {{ $material->store_type == '0' ? 'selected' : '' }} class="form-control">Material Big Store</option>
                                        <option value="1" {{ $material->store_type == '1' ? 'selected' : '' }} class="form-control">Material Medium Store</option>
...
<script src="//ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
    ...
    if ($(this).val() === '0'){
        ...
        var material_bg_store = "<div class='form-group'>"+
                            ...
                            "<select name='code_store' class='form-control'>"+
                                ...
                                "@foreach($material_bg_stores as $material_big_store)"+
                                "<option value='{{$material_big_store->code}}'>{{ $material_big_store->code}}&nbsp;{{ $material_big_store->name}}"+
                                "@endforeach"
                            +
                        "</div>";
        
        $("#dynamic_big_store").append(material_bg_store);
    } 
    if ($(this).val() === '1'){

        var material_md_store = "<div class='form-group'>"+
                            "<label for='code_store'>Material Medium Store<strong style='color: red;'>*</strong></label>"+
                            "<select name='code_store' class='form-control'>"+
                                "<option selected disabled>merci de choisir</option>"+
                                "@foreach($material_md_stores as $material_medium_store)"+
                                "<option value='{{$material_medium_store->code}}'>{{ $material_medium_store->code}}&nbsp;{{ $material_medium_store->name}}"+
                                "@endforeach"
                            +
                        "</div>";
        
        $("#dynamic_big_store").append(material_md_store);
    }
    if ($(this).val() === '2'){
        ...
        var small_store = "<div class='form-group'>"+
                            ...
                            "<select name='code_store' class='form-control'>"+
                                "<option selected disabled>merci de choisir</option>"+
                                "@foreach($material_sm_stores as $material_small_store)"+
