        ...
        <div class="col-sm-6">
            <div class="breadcrumbs-area clearfix">
                ...
                <ul class="breadcrumbs pull-left">
                    ...
                    <li><a href="{{ route('admin.sotb-material-receptions.index') }}">@lang('messages.list')</a></li>
        ...
        <div class="col-12 mt-5">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.sotb-material-reception-without-order.store') }}" method="POST">
                        @csrf
                    <div class="row">
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="store_type" id="store_type">
                                        ...
                                        <option value="2" class="form-control">PETIT STOCK</option>
                                    </select>
                                </div>
                        </div>
                        <div class="col-md-6" id="dynamic_big_store">
                         ...
                         <table class="table table-bordered" id="dynamicTable">  
                            ...
                            <tr>  
                                ...
                                <td><input type="number" name="purchase_price[]" value="{{$data->price}}" class="form-control" step="any" min="0" readonly /></td>
...
<script src="//ajax.googleapis.com/ajax/libs/jquery/1.12.4/jquery.min.js"></script>
    ...
    if ($(this).val() === '0'){
        ...
        var big_store = "<div class='form-group'>"+
                            "<label for='destination_bg_store_id'>GRAND STOCK<strong style='color: red;'>*</strong></label>"+
                            "<select name='destination_bg_store_id' class='form-control'>"+
                                "<option selected disabled>merci de choisir</option>"+
                                "@foreach($destination_bg_stores as $destination_big_store)"+
    ...
    if ($(this).val() === '1'){
        ...
        var medium_store = "<div class='form-group'>"+
                            "<label for='destination_md_store_id'>STOCK INTERMEDIAIRE<strong style='color: red;'>*</strong></label>"+
                            "<select name='destination_md_store_id' class='form-control'>"+
                                "<option selected disabled>merci de choisir</option>"+
                                "@foreach($destination_md_stores as $destination_store)"+
                                "<option value='{{$destination_store->id}}'>{{ $destination_store->code}}&nbsp;{{ $destination_store->name}}"+
                                "@endforeach"
                            +
                        "</div>";
        
        $("#dynamic_big_store").append(medium_store);
    }
    if ($(this).val() === '2'){

        var small_store = "<div class='form-group'>"+
                            "<label for='destination_sm_store_id'>PETIT STOCK<strong style='color: red;'>*</strong></label>"+
                            "<select name='destination_sm_store_id' class='form-control'>"+
                                "<option selected disabled>merci de choisir</option>"+
                                "@foreach($destination_sm_stores as $destination_store)"+
                                "<option value='{{$destination_store->id}}'>{{ $destination_store->code}}&nbsp;{{ $destination_store->name}}"+
                                "@endforeach"
                            +
                        "</div>";
