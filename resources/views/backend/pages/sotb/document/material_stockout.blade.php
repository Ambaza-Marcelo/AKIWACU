            ...
            <div>
                ...
                <div>
                    <div style="float: right; border-top-right-radius: 10px solid black;border-top-left-radius: 10px solid black;border-bottom-right-radius: 10px solid black;border-bottom-left-radius: 10px solid black; background-color: rgb(150,150,150);width: 242px;padding: 20px;">
                        <small>
                           &nbsp;&nbsp; <img src="data:image/png;base64, {!! base64_encode(QrCode::size(50)->generate('eSIGNATURE : '.$stockout_signature.' www.sotb.bi')) !!} ">
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Stockout Number: {{ $stockout_no }}
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Origin: @if($data->origin_bg_store_id){{ $data->originBgStore->name}} @elseif($data->origin_md_store_id){{ $data->originMdStore->name}} @elseif($data->origin_sm_store_id){{ $data->originSmStore->name}} @endif
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Asker: {{ $data->asker }}
                        </small><br>
