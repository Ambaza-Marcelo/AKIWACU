            ...
            <div>
                ...
                <div>
                    <div style="float: right; border-top-right-radius: 10px solid black;border-top-left-radius: 10px solid black;border-bottom-right-radius: 10px solid black;border-bottom-left-radius: 10px solid black; background-color: rgb(150,150,150);width: 242px;padding: 20px;">
                        <small>
                           &nbsp;&nbsp; <img src="data:image/png;base64, {!! base64_encode(QrCode::size(50)->generate('eSIGNATURE : '.$stockin_signature.' www.sotb.bi, Stockin number : '.$stockin_no)) !!} ">
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Stockin Number: {{ $stockin_no }}
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Stockin Type: {{ $data->item_movement_type }}
                        </small><br>
                        <small>
                           &nbsp;&nbsp;Destination : @if($data->destination_bg_store_id){{ $data->destinationBgStore->name}} @elseif($data->destination_md_store_id){{ $data->destinationMdStore->name}} @elseif($data->destination_sm_store_id){{ $data->destinationSmStore->name}} @endif
                        </small><br>
                        <small>
