...
use App\Exports\MaterialSmStoreReportExport;
use Carbon\Carbon;
use PDF;
use Validator;
use Excel;
...
class MaterialSmallReportController extends Controller
{
    //
    public $user;
    ...
    public function index(){
        if (is_null($this->user) || !$this->user->can('material_small_report.view')) {
            ...
            $stores = MaterialSmallStore::all();

        return view('backend.pages.material_small_store_report.index',compact('datas','stores'));
    ...
    public function exportToPdf(Request $request)
    {
        ...
        $store_signature = MaterialSmallStore::where('code',$code_store)->value('store_signature');
        ...
        $pdf = PDF::loadView('backend.pages.document.material_small_store_report',compact('datas','dateTime','setting','start_date','end_date','code_store','store_signature'))->setPaper('a4', 'landscape');

        Storage::put('public/material_small_store_report/'.'RAPPORT_PETIT_STOCK_MATERIEL'.$d1.'_'.$d2.'.pdf', $pdf->output());

        // download pdf file
        return $pdf->download('RAPPORT_PETIT_STOCK_MATERIEL'.$dateTime.'.pdf');
    ...
    {
        return Excel::download(new MaterialSmStoreReportExport, 'RAPPORT_PETIT_STOCK_DES_MATERIELS.xlsx');
    }
}
