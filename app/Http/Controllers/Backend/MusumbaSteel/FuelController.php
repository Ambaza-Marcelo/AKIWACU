...
class FuelController extends Controller
{
    ...
    {
        if (is_null($this->user) || !$this->user->can('musumba_steel_fuel.view')) {
            abort(403, 'Sorry !! You are Unauthorized to view any fuel !');
        }

        $fuels = MsFuel::all();
        return view('backend.pages.musumba_steel.fuel.fuel.index', compact('fuels'));
    }

    public function create()
    {
        if (is_null($this->user) || !$this->user->can('musumba_steel_fuel.create')) {
            abort(403, 'Sorry !! You are Unauthorized to create any fuel !');
        }
        return view('backend.pages.musumba_steel.fuel.fuel.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('admin.ms-fuels.index');
    }

    public function edit($id)
    {
        if (is_null($this->user) || !$this->user->can('musumba_steel_fuel.edit')) {
            abort(403, 'Sorry !! You are Unauthorized to edit any fuel !');
        }

        $fuel = MsFuel::find($id);
        return view('backend.pages.musumba_steel.fuel.fuel.edit', compact('fuel'));
    }
    ...
    {
        if (is_null($this->user) || !$this->user->can('musumba_steel_fuel.delete')) {
