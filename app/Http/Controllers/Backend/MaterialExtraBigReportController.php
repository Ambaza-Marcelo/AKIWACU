...
class MaterialExtraBigReportController extends Controller
{
    //
    public $user;
    ...
    public function index(){
        if (is_null($this->user) || !$this->user->can('material_big_report.view')) {
            ...
            $datas = MaterialExtraBigReport::select(
            ...
            $stores = MaterialExtraBigStore::all();

        return view('backend.pages.material_extra_big_store_report.index',compact('datas','stores'));
    ...
    public function exportToPdf(Request $request)
    {
        ...
        $store_signature = MaterialExtraBigStore::where('code',$code_store)->value('store_signature');
        ...
        $pdf = PDF::loadView('backend.pages.document.material_extra_big_store_report',compact('datas','dateTime','setting','start_date','end_date','code_store','store_signature'))->setPaper('a4', 'landscape');

        Storage::put('public/material_extra_big_store_report/'.'RAPPORT_EXTRA_GRAND_STOCK_MATERIEL'.$d1.'_'.$d2.'.pdf', $pdf->output());

        // download pdf file
        return $pdf->download('RAPPORT_EXTRA_GRAND_STOCK_MATERIEL'.$dateTime.'.pdf');
