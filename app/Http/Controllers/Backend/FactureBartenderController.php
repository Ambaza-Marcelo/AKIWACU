...
class FactureBartenderController extends Controller
{
    ...
    public function create($order_no)
    {
        ...
        return view('backend.pages.invoice_bartender.create',compact('bartender_items','data','setting','orders','order_no','clients'));
    }
    ...
    public function rapportBartender(Request $request)
    {
        ...
        $datas = FactureDetail::select(
                        DB::raw('id,bartender_item_id,invoice_number,invoice_date,item_quantity,item_price,vat,item_price_nvat,customer_name,client_id,item_total_amount'))->where('bartender_order_no','!=','')->where('etat','1')->whereBetween('invoice_date',[$start_date,$end_date])->groupBy('id','bartender_item_id','invoice_date','invoice_number','item_quantity','item_price','vat','item_price_nvat','customer_name','client_id','item_total_amount')->orderBy('invoice_number','asc')->get();
        ...
        $credits = FactureDetail::select(
                        DB::raw('id,bartender_item_id,invoice_number,invoice_date,item_quantity,item_price,vat,item_price_nvat,customer_name,client_id,item_total_amount'))->where('bartender_order_no','!=','')->where('etat','01')->whereBetween('invoice_date',[$start_date,$end_date])->groupBy('id','bartender_item_id','invoice_date','invoice_number','item_quantity','item_price','vat','item_price_nvat','customer_name','client_id','item_total_amount')->orderBy('invoice_number','asc')->get();
