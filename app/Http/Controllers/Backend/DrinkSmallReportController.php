...
class DrinkSmallReportController extends Controller
{
    ...
    public function exportToPdf(Request $request)
    {
        ...
        $datas = DrinkSmallReport::select(
                        DB::raw('id,created_at,drink_id,type_transaction,document_no,created_by,quantity_stock_initial,value_stock_initial,quantity_stockin,value_stockin,quantity_inventory,value_inventory,quantity_inventory_ml,value_inventory_ml,quantity_reception,value_reception,quantity_transfer,value_transfer,quantity_stockout,value_stockout,quantity_sold,value_sold,quantity_stock_final,value_stock_final'))->whereBetween('date',[$start_date,$end_date])->where('code_store',$code_store)->where('drink_id',$drink_id)->groupBy('id','created_at','drink_id','type_transaction','document_no','created_by','quantity_stock_initial','value_stock_initial','quantity_stockin','value_stockin','quantity_inventory','quantity_inventory_ml','value_inventory','value_inventory_ml','quantity_reception','value_reception','quantity_transfer','value_transfer','quantity_stockout','value_stockout','quantity_sold','value_sold','quantity_stock_final','value_stock_final')->orderBy('id','asc')->get();
